...
pandas (@pandas), altair (@altair), click (@Manual), 
as well as Quarto (@Allair_Quarto_2022). 
...
Additionally, note that in @table-1 we consider the sample standard deviation of the number of horses during the same time span as @fig-horse.

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-pop-plot}


In @fig-pop-plot we zoom in and look at the province of `{python} largest_sd`, which had the largest spread of values in terms of standard deviation.
